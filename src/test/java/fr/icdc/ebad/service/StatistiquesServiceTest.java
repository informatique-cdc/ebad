...
//        when(logBatchRepository.countBatchByDay(any())).thenReturn(countBatchByDay);
        ...
        StatisticsDto statistiquesDto = statistiquesService.generationStatistiques();

        assertEquals(1L, statistiquesDto.getBatchsNbrs(), 0);
//        assertEquals(2L, statistiquesDto.getBatchsRunnedNbr(), 0);
//        assertEquals(3L, statistiquesDto.getTpsMoyenBatch(), 0);
        assertEquals(4L, statistiquesDto.getUsersNbr(), 0);
        assertEquals(6L, statistiquesDto.getApplicationsNbr(), 0);
